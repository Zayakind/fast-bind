...
### Добавлено
- 🛡️ **Улучшенная совместимость с антивирусными программами**
  - Новая feature `console` для сборки с консольным окном (менее подозрительно для антивирусов)
  - Секция в README.md о проблемах с антивирусами и их решении

### Изменено
- 🔧 **Смягчены агрессивные оптимизации сборки**
  - Изменено `lto = true` на `lto = "thin"` (менее агрессивная оптимизация)
  - Убрано `strip = true` для сохранения отладочной информации
  - Добавлено `debug = true` в release профиль для лучшей совместимости
  - Условная компиляция `windows_subsystem` с поддержкой feature `console`

### Удалено
- ❌ **Неиспользуемые зависимости** для уменьшения размера сборки и времени компиляции:
  - `tokio` - приложение полностью синхронное
  - `anyhow` - используется `thiserror` вместо него
  - `rfd` - диалоговые окна не реализованы

### Техническое
- Добавлен build dependency `winres` для встраивания версионной информации
- Создание нескольких вариантов сборки: стандартная, с консолью, debug
- Папка `releases/` добавлена в `.gitignore`
- Улучшена документация для пользователей и разработчиков
- Оптимизация зависимостей: удаление 3 неиспользуемых пакетов
